...
describe("application routing", () => {
  ...
  it("navigates to an intentionally empty Stage 5 module", async () => {
    vi.spyOn(globalThis, "fetch").mockRejectedValue(new TypeError("offline"));
    const user = userEvent.setup();
    const router = createMemoryRouter(routes, { initialEntries: ["/app"] });
    render(<RouterProvider router={router} />);
    await user.click(screen.getByRole("link", { name: "Registry" }));
    expect(await screen.findByRole("heading", { name: "Registry", level: 2 })).toBeInTheDocument();
    expect(screen.getByText(/will be connected in Stage 5/i)).toBeInTheDocument();
    await waitFor(() => expect(router.state.location.pathname).toBe("/app/registry"));
  });
