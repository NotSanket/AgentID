...
export function NetworkStatus({ compact = false }: { compact?: boolean }) {
  ...
  const degraded = health.phase === "offline" && data !== null;
  const label = loading ? "CHECKING SYSTEM" : online ? "SYSTEM ONLINE" : degraded ? "SYSTEM DEGRADED" : "SYSTEM OFFLINE";
  ...
  return (
    <button className={`network-status ${online ? "is-online" : loading ? "is-loading" : "is-offline"}`} type="button" onClick={health.refresh} title={online ? "Refresh system status" : health.error ?? "Refresh system status"} aria-label={`${label}. Refresh system status`}>
      ...
      {!compact && <span className="network-copy"><strong>{label}</strong><small>{online && data ? `${data.blockchain.network.toUpperCase()} · CHAIN ${data.blockchain.chainId}` : degraded ? "BACKEND CONNECTED · BLOCKCHAIN UNAVAILABLE" : "BACKEND UNREACHABLE"}</small></span>}
