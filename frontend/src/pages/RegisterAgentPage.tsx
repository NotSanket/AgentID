    ...
    <div className="page-stack stage5-page registration-page">
      ...
      <section className="wizard-panel">
        ...
        {step === 2 && <div className="wizard-content">
          <div className="section-heading"><WalletCards /><div><h3>Choose controlling wallet</h3><p>The transaction sender becomes the permanent controlling wallet.</p></div></div>
          <div className="ownership-grid">
            ...
            <div className="ownership-card local-wallet-card"><Laptop /><strong>Local Demo Wallet</strong><small>Development only · Hardhat 31337</small><div className="demo-wallet-list">{session.demoWallets.map((wallet) => <button key={wallet.address} type="button" disabled={!wallet.available} className={session.address === wallet.address ? "is-selected" : ""} onClick={() => session.selectDemoWallet(wallet)}><span><strong>{wallet.label}</strong><small>{wallet.available ? "AVAILABLE" : `IN USE · ${wallet.assignedAgentId}`}</small></span><code>{`${wallet.address.slice(0, 8)}…${wallet.address.slice(-6)}`}</code></button>)}</div></div>
          </div>
