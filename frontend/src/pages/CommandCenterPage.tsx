...
export function CommandCenterPage() {
  ...
  return <div className="page-stack command-center-page stage5-page">
    ...
    <section className="dashboard-grid">
      <ComponentSlot eyebrow="Unified real-time evidence" title="Recent platform activity">{failed ? <EmptyState kind="network" title="Activity unavailable" description="The backend may be waking. Refresh after the production service is online." /> : <ActivityFeed items={[...lifecycle.map((data) => ({ kind: "lifecycle" as const, data })), ...audit.map((data) => ({ kind: "audit" as const, data }))]} />}</ComponentSlot>
      <ComponentSlot eyebrow="Safe next actions" title="Quick actions"><div className="quick-actions"><QuickAction to="/app/register" icon={KeyRound} title="Register an agent" label="Issue on-chain identity" /><QuickAction to="/app/communication" icon={RadioTower} title="Send authenticated request" label="Open communication" /><QuickAction to="/app/security" icon={ShieldAlert} title="Open Security Lab" label="Development-only guarded scenarios" /><QuickAction to="/app/trust-graph" icon={GitBranch} title="Open Trust Graph" label="Observed identity relationships" /><QuickAction to="/app/explorer" icon={SearchCode} title="Inspect Explorer" label="Blocks, events, and receipts" /><QuickAction to="/app/analytics" icon={BarChart3} title="View Analytics" label="Persistent real metrics" /></div></ComponentSlot>
...
function DemoReadiness({ health }: { health: ReturnType<typeof useSystemHealth> }) {
  ...
  const blockchain = Boolean(data?.blockchain.connected && data.blockchain.chainId === data.blockchain.expectedChainId);
  ...
  const localDemo = data?.blockchain.chainId === 31337;
  const persistence = Boolean(data && (localDemo
    ? data.persistenceMode === "IN_MEMORY" || data.supabaseConnected
    : data.persistenceMode === "SUPABASE" && data.supabaseConnected));
  const signingPolicy = Boolean(data && (localDemo ? signing : !signing));
  const ready = backend && blockchain && contract && persistence && signingPolicy;
  const checking = health.phase === "loading";
  const title = checking ? "CHECKING SYSTEM" : ready ? localDemo ? "DEMO READY" : "PUBLIC SYSTEM READY" : localDemo ? "DEMO NOT READY" : "PUBLIC SYSTEM DEGRADED";
  return <section className={`demo-readiness panel ${ready ? "is-ready" : "is-not-ready"}`} aria-label={localDemo ? "Local demo readiness" : "Public system readiness"}>
    <header><div><p className="eyebrow">Teacher presentation check</p><h3>{title}</h3><p>{ready ? localDemo ? "All dependencies required for the guarded local demonstration are healthy." : "Sepolia identity, persistence, and production signing guardrails are healthy." : checking ? "Checking backend and identity dependencies." : health.error ?? "One or more required dependencies need attention."}</p></div><StatusBadge tone={checking ? "pending" : ready ? "verified" : "blocked"} label={checking ? "CHECKING" : ready ? "READY" : "ATTENTION"} /></header>
    <div className="demo-readiness-grid">
      ...
      <ReadinessItem label="Persistence" ready={persistence} detail={persistence ? data?.persistenceMode === "SUPABASE" ? "SUPABASE CONNECTED" : "IN-MEMORY READY" : localDemo ? "UNAVAILABLE" : "SUPABASE UNAVAILABLE"} />
      <ReadinessItem label={localDemo ? "Demo signing" : "Production signing"} ready={signingPolicy} detail={localDemo ? signing ? "LOCAL ONLY" : "DISABLED" : signing ? "UNSAFE" : "BROWSER WALLET ONLY"} />
    </div>
  </section>;
}
